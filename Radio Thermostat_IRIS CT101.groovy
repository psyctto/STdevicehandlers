/*
 *  Radio Thermostat / IRIS
 *  CT101
 *  (Used Smartthings "CT100 Thermostat" 20170822 as base design)
 *  
 *  Version history
 *		20181024 v0.1  initial release; cycle monitoring/storage calculates and stores last time on as well as duration, prior data is also stored and accessible.
 *  
 *
 *  ToDo
 *  	Various cleanup
 *		Fix set point display in multiTile between control arrows, seems to be displaying current temperature.
 */
metadata {
	// Automatically generated. Make future change here.
	definition (name: "Radio Thermostat_IRIS CT101", namespace: "psyctto", author: "psyctto") {
		capability "Actuator"
        ...
        attribute "lastOpState", "string"  //psyctto
        attribute "lastTimeOn", "number"  //psyctto
        attribute "lastTimeOff", "number"  //psyctto
        attribute "previousTimeOff", "number"  //psyctto
        attribute "lastRunDetail", "string"  //psyctto
        attribute "preiousRunDetail", "string"  //psyctto
        attribute "lastRunDuration", "string"  //psyctto
        attribute "previousRunDuration", "string"  //psyctto
        attribute "runDetailDisplayType", "string"  //psyctto
		...
		command "poll"
        command "togglePreviousRunDetail"  //psyctto
        command "updateRunSinceDetail"  //psyctto

		fingerprint deviceId: "0x08", inClusters: "0x43,0x40,0x44,0x31,0x80,0x85,0x60"
		fingerprint mfr:"0098", prod:"6401", model:"0107", deviceJoinName: "2Gig CT101 Programmable Thermostat"
	}
 ...
 */
		standardTile("mode", "device.thermostatMode", width:2, height:1, inactiveLabel: false, decoration: "ring") {
			state "off", action:"switchMode", nextState:"...", icon: "st.thermostat.heating-cooling-off"
			...
			state "auto", action:"switchMode", nextState:"...", icon: "st.thermostat.auto", backgroundColor:"#90d2a7"
			state "emergency heat", action:"switchMode", nextState:"...", icon: "st.thermostat.emergency-heat", backgroundColor:"#f1d801"
			state "...", label: "Updating...",nextState:"...", backgroundColor:"#ffffff"
		}
		standardTile("fanMode", "device.thermostatFanMode", width:2, height:1, inactiveLabel: false, decoration: "ring") {
			state "auto", action:"switchFanMode", nextState:"...", icon: "st.thermostat.fan-auto", backgroundColor:"#90d2a7"
			state "on", action:"switchFanMode", nextState:"...", icon: "st.thermostat.fan-on", backgroundColor:"#1e9cbb"
			state "circulate", action:"switchFanMode", nextState:"...", icon: "st.thermostat.fan-circulate", backgroundColor:"#1e9cbb"
			state "...", label: "Updating...", nextState:"...", backgroundColor:"#ffffff"
		}
...
//			state "thermostatOperatingState", label:'${currentValue}', backgroundColor:"#ffffff"
//		}
		standardTile("refresh", "device.thermostatMode", width:2, height:1, inactiveLabel: false, decoration: "ring") {
			state "default", action:"refresh.refresh", icon:"st.secondary.refresh-icon", backgroundColor: "#00a0dc"
		}
        valueTile("runDetail", "device.runDetail",width: 5, height: 1, decoration:"flat") {  //psyctto
			state "default", label:'${currentValue}', action:"togglePreviousRunDetail", backgroundColor:"#ffffff"  //psyctto
		}  //psyctto
        valueTile("runDuration", "device.runDuration",width: 1, height: 1, decoration:"flat") {  //psyctto
			state "default", label:'${currentValue}', backgroundColor:"#ffffff"  //psyctto
		}  //psyctto
        
		main "temperature"
//		details(["temperature", "lastRunDetail", "lowerHeatingSetpoint", "heatingSetpoint", "raiseHeatingSetpoint", "lowerCoolSetpoint",
//				"coolingSetpoint", "raiseCoolSetpoint", "mode", "fanMode", "humidity", "thermostatOperatingState", "refresh"])
		details(["temperature", "runDetail", "runDuration", "mode", "fanMode", "refresh"])
	}
}
...
def initialize() {
	unschedule()
    runEvery1Minute(updateRunSinceDetail)
...
def zwaveEvent(physicalgraph.zwave.commands.thermostatoperatingstatev2.ThermostatOperatingStateReport cmd) {
	...
	switch (cmd.operatingState) {
		case physicalgraph.zwave.commands.thermostatoperatingstatev2.ThermostatOperatingStateReport.OPERATING_STATE_IDLE:
			if(state.lastOpState == "heating"){  //psyctto            	
            	state.lastTimeOff = now()  //psyctto
                timeOperating = (state.lastTimeOff - state.lastTimeOn) / 60000  //psyctto
                lastDateTime = new Date(state.lastTimeOn).format("MM-dd-yy h:mm a", location.timeZone)  //psyctto
                state.lastRunDetail = "Last Run On " + lastDateTime  //psyctto
                sendEvent(name: "runDetail", value: state.lastRunDetail, displayed: false)  //psyctto
                state.lastRunDuration = String.format("%3.2f",timeOperating) + "\nmins"  //psyctto
                sendEvent(name: "runDuration", value: state.lastRunDuration, displayed: false)  //psyctto
            }  //psyctto
            map.value = "idle"
            state.lastOpState = map.value  //psyctto
			break
		case physicalgraph.zwave.commands.thermostatoperatingstatev2.ThermostatOperatingStateReport.OPERATING_STATE_HEATING:
			if(state.lastOpState == "idle"){  //psyctto
            	//Store previous run details
                state.previousRunDetail = "***Previous Run***\n" + state.lastRunDetail  //psyctto
                state.previousRunDuration = state.lastRunDuration  //psyctto
                state.previousTimeOff = state.lastTimeOff  //psyctto
            	state.lastTimeOn = now()  //psyctto
                lastDateTime = new Date(state.lastTimeOn).format("MM-dd-yy h:mm a", location.timeZone)  //psyctto
                state.lastRunDetail = "Currently Running\nSince ${lastDateTime}"  //psyctto
                sendEvent(name: "runDetail", value: state.lastRunDetail, displayed: false)  //psyctto
                state.lastRunDuration = "Just\nStarted"
                sendEvent(name: "runDuration", value: state.lastRunDuration, displayed: false)  //psyctto
            }  //psyctto
            map.value = "heating"
            state.lastOpState = map.value  //psyctto
...
def togglePreviousRunDetail() {
	log.debug "togglePreviousRunDetail()"
    
	if (state.runDetailDisplayType == "current") {
		state.runDetailDisplayType = "previous"
        sendEvent(name: "runDetail", value: state.previousRunDetail, displayed: false)
        sendEvent(name: "runDuration", value: state.previousRunDuration, displayed: false)
	}
	else {
		state.runDetailDisplayType = "current"
        sendEvent(name: "runDetail", value: state.lastRunDetail, displayed: false)
        sendEvent(name: "runDuration", value: state.lastRunDuration, displayed: false)
	}
    updateRunSinceDetail()
}

def updateRunSinceDetail() {
	def epochTimeString = now()
	def lastTimeSinceOn = (epochTimeString - state.lastTimeOff) / 60000  //psyctto
    def previousTimeSinceOn = (epochTimeString - state.previousTimeOff) / 60000  //psyctto
    def lastRunDetailTime = state.lastRunDetail + "\n${String.format("%3.2f",lastTimeSinceOn)}mins Ago"
    def previousRunDetailTime = state.previousRunDetail + "\n${String.format("%3.2f",previousTimeSinceOn)}mins Ago"    
    def timeOperating
    def runDuration
    
    log.debug "updateRunSinceDetail: ${state.runDetailDisplayType}, ${state.lastOpState}"
    
    if (state.runDetailDisplayType == "current"){
    	if (state.lastOpState == "heating"){
        	timeOperating = (now() - state.lastTimeOn) / 60000  //psyctto
    		runDuration = String.format("%3.2f",timeOperating) + "\nmins"  //psyctto
    		sendEvent(name: "runDuration", value: runDuration, displayed: false)  //psyctto
            sendEvent(name: "runDetail", value: state.lastRunDetail, displayed: false)  //psyctto
        }
        else{
    		sendEvent(name: "runDetail", value: lastRunDetailTime, displayed: false)  //psyctto
        }
    }
    else{
    	sendEvent(name: "runDetail", value: previousRunDetailTime, displayed: false)  //psyctto
    }
}
